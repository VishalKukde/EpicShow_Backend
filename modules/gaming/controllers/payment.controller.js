...
export const preparePayment = async (req, res) => {
  try {
    ...
    const resolvedItemId = itemId || gamingId || movieId;
    ...
    const gaming = await Gaming.findById(resolvedItemId).select("price");
    if (!gaming) {
...
export const createOrder = async (req, res) => {
  ...
  try {
    ...
    const resolvedItemId = itemId || gamingId || movieId;
    const resolvedShowType = showType || "gaming";
    ...
    const gaming = await Gaming.findById(resolvedItemId).select("price").session(session);
    if (!gaming) {
      await session.abortTransaction();
      session.endSession();
      return res.status(404).json({ message: "Gaming show not found" });
        ...
        [
          {
            ...
            balanceAfter,
            status: "success",
            note: `Gaming booking payment (${booking._id})`,
            booking: booking._id,
            payment: payment._id,
          },
        ],
